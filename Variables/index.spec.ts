...
describe("dom.Variables", () => {
	it.each([
		...
		{
			input: [{ user: { prefs: { theme: "dark" } } }, { user: { prefs: null } }],
			expected: { user: { prefs: null } },
		},
	...
	it.each([
		...
		// Complex nesting
		{
			variables: {
				app: {
					name: "MyApp",
					config: { env: "prod", features: { auth: true, cache: { enabled: true, ttl: 300 } } },
				},
			},
	...
	it.each([
		...
		// Complex path-based logic
		{
			...
			mapper: (value: any, path: string[]) => {
				if (path.includes("profile") && typeof value === "string")
					return value.toUpperCase()
				if (path.includes("notifications") && typeof value === "boolean")
					return !value
				return value
			},
			expected: {
